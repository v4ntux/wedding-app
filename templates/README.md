...
=======
1. **Открытие** — вставить `{{{envelope}}}` первым элементом `<body>`. Движок (`src/experience.js`) сам покажет полноэкранный конверт, подставит имена на карточку и уйдёт. Тема конверта берётся по id шаблона (см. `THEMES` там же). Старые `.envx`/`#env` продолжают работать для legacy-шаблонов.
>>>>>>> theirs
