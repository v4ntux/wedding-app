...
import { luxuryBlocks } from "./luxuryBlocks.js";
...
export function buildData(app, guestName = null, tpl = null) {
  ...
  const mapEnabled =
    app.map_enabled === undefined ? true : Boolean(Number(app.map_enabled));
  const links = mapEnabled
    ? mapsLinks(app.lat, app.lng)
    : { google: "", yandex: "" };
  ...
  if (app.music_type === "preset") {
    ...
    music = {
      url,
      name: L.music,
      playable: /\.(mp3|ogg|m4a|wav)([?#]|$)/i.test(url),
    };
  }
  if (music) {
  ...
  photos = photos.map((p) =>
    String(p).startsWith("/") ? String(p) : `/uploads/${p}`,
  );
  ...
  return { ...data, ...luxuryBlocks(data, tpl) };
}
...
export function renderInvitation(app, guestName = null) {
  ...
  if (!tpl) {
    ...
    console.warn(
      `[render] шаблон "${app.template_id}" не найден — рендерю "${tpl.id}"`,
    );
  }
  return renderTemplate(tpl.tree, buildData(app, guestName, tpl));
}

// A small demo label keeps typography readable and leaves text selectable.
const WATERMARK = `<div class="demo-watermark" aria-label="Demo">NVATE · DEMO</div>`;
  ...
  const sample = {
    ...
    wedding_date:
      ...
      !Number.isNaN(Date.parse(opts.date))
        ? opts.date
        : `${new Date().getFullYear() + 1}-09-19`,
    wedding_time: /^([01]\d|2[0-3]):[0-5]\d$/.test(opts.time || "")
      ? opts.time
      : "18:00",
    address:
      String(opts.address ?? "")
        .slice(0, 140)
        .trim() || "To‘yxona «Navro‘z», Mang‘it",
...
var invitation=document.getElementById('invitation');if(invitation)invitation.inert=false;
